...
function ellak_edu_fos_synch_posts(){
    ...
    if(file_exists($custom_base_dir.'/edu_fos_final.csv')){
        ...
        fgetcsv($edu_fos_csv); //the title
        //fgetcsv($edu_fos_csv); //the blank line
        ...
        while (! feof($edu_fos_csv)){
            ...
            if(mb_strpos($thematiki, '#')){
                ...
                error_log('thematiki size: '.sizeof($thematiki));
            }
            
            $antikimeno=$edu_fos_txt_line[2];
            //check for multiple terms in the antikimeno field, split on # delimiter
            if(mb_strpos($antikimeno, '#')){
                $antikimeno=mb_split('#', $vathmida);
                error_log('antikimeno size: '.sizeof($antikimeno));
            }
            ...
            if(mb_strpos($vathmida, '#')){
                ...
                error_log('vathmida size: '.sizeof($vathmida));
            }
